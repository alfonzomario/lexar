...
import { Calculator as CalcIcon, Calendar as CalendarIcon, AlertCircle, Info, ArrowRight } from 'lucide-react';
...
export function Calculator() {
  ...
  const calculateDeadline = (e: React.FormEvent) => {
    ...
    // Skip weekends for start date if hábiles
    if (act.type === 'hábiles') {
      while (computeStart.getDay() === 0 || computeStart.getDay() === 6) {
        computeStart.setDate(computeStart.getDate() + 1);
      }
    }
    ...
    while (gracePeriod.getDay() === 0 || gracePeriod.getDay() === 6) {
      gracePeriod.setDate(gracePeriod.getDate() + 1);
    }
    ...
    >
          ...
          {result ? (
            ...
            <div className="h-full bg-stone-50 rounded-3xl border border-stone-200 border-dashed flex flex-col items-center justify-center p-12 text-center text-stone-400">
              ...
              <p className="text-sm mt-2 max-w-sm">
                El sistema calculará automáticamente los días hábiles, excluyendo fines de semana.
              </p>
            </div>
          )}
        </div>
      ...
      <div className="bg-amber-50 border border-amber-200 rounded-2xl p-6 flex gap-4 items-start">
        ...
        <div className="text-sm text-amber-800 space-y-2">
          ...
          <p>
            El cálculo <strong>no incluye feriados nacionales ni asuetos judiciales específicos</strong> (ferias, días del trabajador judicial, etc.). Es responsabilidad exclusiva del profesional verificar los plazos aplicables según los códigos procesales y acordadas vigentes.
          </p>
        </div>
      </div>
    </motion.div>
